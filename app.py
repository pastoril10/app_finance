...
def home():
    ...
    with col2:
        st.image('./imagem/logo.jpeg', width=500)
        st.markdown('---')
        # st.title('App Financeiro')
        st.markdown("<h1 style='text-align: center; color: red;'>App Finance</h1>", unsafe_allow_html=True)
        st.markdown('---')
...
def panorama():
    ...
    # Seleção de Ações
    acoes_listadas = pd.read_csv("./data/acoes-listadas.csv")
    colunas = acoes_listadas.columns
    acoes_listadas_na_B3 = acoes_listadas.reset_index()
    acoes_listadas_na_B3.rename(columns={acoes_listadas_na_B3.columns[0]:colunas[0],
                                         acoes_listadas_na_B3.columns[1]:colunas[1],
                                         acoes_listadas_na_B3.columns[2]:colunas[2],
                                         acoes_listadas_na_B3.columns[3]:colunas[3],
                                         acoes_listadas_na_B3.columns[4]:colunas[4]}, inplace=True)
    
    acoes_listadas_na_B3["Código"] = acoes_listadas_na_B3["Código"] + ".SA"

    acao = st.selectbox('Selecione a ação', acoes_listadas_na_B3["Código"].sort_values())
...
def mapa_mensal():
    ...
    if opcao == 'Indices':
        with st.form(key='form_indice'):
            indice = st.selectbox('Indice', ['Bovespa', 'S&P500', 'NASDAQ', 'DAX', 
                                             'FTSE 100', 'Cruid Oil', 'Gold', 'BITCOIN', 'ETHEREUM'])
            analisar = st.form_submit_button('Analisar')
            ticker = dict_tickers[indice]

    else:
        with st.form(key='form_acoes'):
            ticker = st.selectbox('Ações', acoes_listadas_na_B3["Código"].sort_values())
            analisar = st.form_submit_button('Analisar')
    ...
    if  analisar:
        ...
        if opcao == 'Indices':       
            retornos = yf.Ticker(ticker).history(start=data_inicial, end=data_final,
                                                interval='1d')['Close'].pct_change()
        else:       
            retornos = yf.Ticker(ticker).history(start=data_inicial, end=data_final,
                                                interval='1d')['Close'].pct_change()
...
# Função principal
def main():
    st.sidebar.image('./imagem/logo.jpeg', width=250)
    st.sidebar.markdown("<h1 style='text-align: center'>App Finance</h1>", unsafe_allow_html=True)
    st.sidebar.markdown('---')
